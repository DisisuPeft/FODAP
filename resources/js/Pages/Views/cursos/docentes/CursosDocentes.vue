...
const confirm_inscripcion = (item) => {
    ...
    if (item.id_departamento !== props.auth.user.departamento_id) {
        notify(
            "Atención!",
            "info",
            "Este curso es externo a su departamento de adscripción. Acuda con el jefe del departamento que propuso el curso para que lo inscriba."
        );
    } else {
        Swal.fire({
...
const submit = (item) => {
    form.post(route("inscripcion.docente", item), {
        ...
        onError: () => {
            errorMsg(
                "Error",
                "Ha ocurrido un error al intentar inscribirse, notificar al area de desarrollo academico"
            );
        },
    });
};
...
<template>
    ...
    <AuthenticatedLayout>
                ...
                <div class="p-4 mt-7 sm:p-8 bg-white shadow-2xl sm:rounded-lg">
                    ...
                    >
                        <template v-slot:default="{ item }">
                            ...
                            <div
                                class="max-w-2xl mx-auto overflow-hidden md:max-w-4xl"
                            >
                                <div class="md:flex">
                                    ...
                                    <div class="p-8">
                                        ...
                                        <p
                                            class="block mt-1 text-sm leading-tight font-medium text-black hover:text-blue-400 hover:underline"
                                            @click="openDialog(item)"
                                        >
                                            CLICK PARA CONOCER MAS DEL CURSO…
                                        </p>
                                    ...
                                    <div class="flex justify-center p-2">
                                        ...
                                        <div v-else-if="item.estado !== 2">
                                            <secondary-button
                                                type="submit"
                                                @click="
                                                    confirm_inscripcion(item)
                                                "
                                                >Inscribirse</secondary-button
                                            >
                                        </div>
                                    </div>
                                </div>
        ...
        <template v-else>
            <div class="mx-auto sm:px-6 lg:px-8 space-y-6">
                <div class="p-4 mt-7 sm:p-8 bg-white shadow sm:rounded-lg">
                    ...
                    >
                        <strong
                            >Actualmente no hay cursos que visualizar !Pronto
                            deberian ser agregados!</strong
                        >
                    </v-alert>
                </div>
            </div>
        ...
        <InfoDialog
            ...
            @update:modelValue="dialogInfo = $event"
        ></InfoDialog>
    </AuthenticatedLayout>
</template>
